...
def main():
    ...
    try:
        contact_plan = ContactPlan(1000, 10, "tests/resources/03_tvg.json")
    except Exception as e:
        print(
            colored(
                "Generating contact plan based on tvg json file "
                "failed!", 'red'))
        sys.exit(1)

    print(
        colored("Generating contact plan based on tvg json file "
                "succeeded!", 'green'))
